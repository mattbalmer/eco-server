...
namespace Eco.Mods
{
    ...
    {
        [ChatCommand("Performs commands for skills")]
        ...
        public static void LevelUpUser(User user, string name = null)
        {
            ...
            user.Player.Msg(Localizer.Format("{0} leveled-up to level {1}", targetUser.UILink(), targetUser.SpecialtyPoints));
        }

        [CITest(clientDependent: true)]
        [ChatSubCommand("Skills", "Levels all skills up 1 level at a time (no chunks).", ChatAuthorizationLevel.Developer)]
        ...
        [ChatSubCommand("Skills", "Remove restrictions and gives you every skill at max level.", "creative", ChatAuthorizationLevel.Developer)]
        ...
        [ChatSubCommand("Inventory", "Adds random items to the user's inventory.", ChatAuthorizationLevel.Developer)]
        ...
        [CITest]
        [ChatSubCommand("QA", "Lists the items that cannot be crafted from some set of other items.  Note that many of these are not meant to be crafted, e.g. hips and stone.", ChatAuthorizationLevel.Developer)]
        ...
        [ChatSubCommand("Craft", "Gets the resources needed to craft something", ChatAuthorizationLevel.Developer)]
