var MapControls = function(){
	...
	var secondsToString = function(seconds, br) {
		...
		//+1 so day can't be 0
		if (br == 'br') {
			return Math.ceil((1 + seconds) / 86400) + "<br>" + date;
		} else {
			return Math.ceil((1 + seconds) / 86400) + ", " + date;
		}
	}
	...
	this.bindEvents = function(){
		...
		$("#map-toggle-play").click(function () {
			let newState = ! mainMap.settings.pause;
		...
		$(document).on('click', '.fancy-add-map', function() {
			...
			let elem = $(".map-layer-select");
			elem = elem.offset();

			$('html, body').animate({scrollTop: elem.top - 200});

			let view = document.getElementById("main-map");
		...
		$(document).on('click', '.remove-map', function() {
			...
			$('.fancy-add-map').html('Add Map').attr('data-action', 'add');

			//$('#graph-data').val('');

			$("#map-control-wrapper").slideUp();
			$("#main-map").slideUp();
